...
class DreamIDV:
    ...
    def __init__(
        ...
        # main device for DiT/VAE
        self.device = torch.device(f"cuda:{device_id}") if torch.cuda.is_available() else torch.device("cpu")

        # T5 run device can be controlled from config (set by Loader UI)
        # allowed: "cpu", "cuda:0", "cuda:1", ...
        t5_dev_str = getattr(config, "t5_device", None)
        if t5_dev_str is None and isinstance(config, dict):
            t5_dev_str = config.get("t5_device", None)
        if t5_dev_str is None:
            # backward compatible fallback: keep original behavior controlled by t5_cpu flag
            t5_dev_str = "cpu" if t5_cpu else str(self.device)

        self.t5_device = torch.device(t5_dev_str) if isinstance(t5_dev_str, str) else t5_dev_str
        print(f"[DreamIDV] t5_device selected by UI: {self.t5_device}")


        # keep legacy flag but derive from actual device
        self.t5_cpu = (str(self.t5_device) == "cpu")
        ...
        self.text_encoder = T5EncoderModel(
            ...
            device=self.t5_device,  # <-- use UI-selected device
        ...
        state = torch.load(dreamidv_ckpt, map_location="cpu")
    ...
    def generate(self,
        ...
        # ---- T5 encode on selected device (cpu / cuda:0 / cuda:1) ----
        t5_run = self.t5_device

        # ensure T5 weights on correct device
        self.text_encoder.model.to(t5_run)

        # run encoder on t5_run
        context = self.text_encoder([input_prompt], t5_run)

        # move context to main device for DiT
        context = [t.to(self.device) for t in context]

        # optional offload T5 after encoding
        if offload_model and str(t5_run) != "cpu":
